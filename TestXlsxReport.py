...
workbook = xlsxwriter.Workbook(ReportPath)  # 生成的报告的路径
...
<table width="800" border="0" cellspacing="0" cellpadding="4">
 <tr>
    <td bgcolor="#CECFAD" height="20" style="font-size:20px">接口自动化测试报告  <a href="http://101.37.15.193:8081/view/接口自动化测试报告">
 </tr>
 <tr>
...
try:
  ...
  mail_content = msg%dict(yonglizongshu=str(len(TestReport)),tongguoyongli=str(hpassnum),shibaiyongli=str(
    len(TestReport) - hpassnum),tongguolv=str(round(hpassnum/len(TestReport),2)*100),test_net=testurl,timenow=timenow,neirong='【'
  )
  mail_title='[接口自动化测试报告]'
  attchments=set(
       [ReportPath])
  logger.info('正在发送测试报告邮件...')
  mymail.send_mail(mail_title,mail_content,attchments)
